...
- `latch_addr` -- защёлкнуть выбранное значение в `addr_reg`;
  - Управляющий сигнал ControlUnit
  - Значение из буферного регистра
  - Собственное инкрементированное значение регистра
...
- `sig_wr` -- записать значение аккумулятора в память:
- `sig_out` -- записать значение аккумулятора в устройство вывода по указанному порту.
  - Строкой
  - Числом
- `latch_br` -- защёлкнуть значение буферного регистра 
  - Берётся значение из аккумулятора
  - результат операции в алу
  - собственное дикрементированное значение регистра

Сигналы для ControlUnit:

- `Z` -- отражает наличие нулевого значения в аккумуляторе.
- `BR_N` -- отражает наличие отрицательного значения в буферном регистре.
- `BR_Z` -- отражает наличие нулевого значения в буферном регистре.
...
- Counter реализован неявно внутри методов класса ControlUnit
...
- Количество инструкций для моделирования лимитировано (100k).
- Остановка моделирования осуществляется при:
    - превышении лимита количества выполняемых инструкций;
    - превышении лимита памяти (объема доступных ячеек)
    - превышении лимита в ячейке памяти (значение вышло из диапазона: [-2^31; 2^31])
...
DEBUG:root:Total instructions: 15
DEBUG:root:Total ticks: 192
...
| Шляпников Александр Дмитриевич | hello      | 1    | -         | 4           | 4         | 30       | -       |
| Шляпников Александр Дмитриевич | cat        | 5    | -         | 12          | 111       | 192      | -       |
| Шляпников Александр Дмитриевич | hello_user | 3    | -         | 15          | 15        | 192      | -       |
